...
export class ErrorFormatter {
	static format(error: any): void {
		...
		if (error instanceof BunstoneError) {
			...
			console.error(`${colors.yellow}${colors.bold}Message   :${colors.reset} ${error.message}`);
		...
		console.error(`\n${colors.red}${line}${colors.reset}`);
		console.error(`${colors.red} Process exited due to initialization error.${colors.reset}`);
		console.error(`${colors.red}${line}${colors.reset}\n`);
	}
}
